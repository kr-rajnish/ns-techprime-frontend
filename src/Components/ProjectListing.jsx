...
const ProjectListing = () => {
  ...
  const fetchData = (page, query, sortBy) => {
    axios
      .get(
        `https://ill-red-lemur.cyclic.app/project?limit=${10}&page=${page}&filter=${query}&sort=${sortBy}`
      )
      .then((res) => {
        // console.log(res);
  ...
  const handleUpdate = (value, id) => {
    axios
      .put(
        `https://tech-prime-backend.onrender.com/project/updateStatus?limit=${10}&page=${page}&filter=${query}&sort=${sortBy}`,
        {
          Status: value,
          id: id,
        }
